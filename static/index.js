...
function dontRemindMe(checkbox) {
  if (checkbox.checked) {
    localStorage.setItem('followUsDismissed', 'true');
    // Close modal after a short delay
    setTimeout(closeFollowUsModal, 300);
  } else {
...
// Prevent modal from opening when PWA modal is open
const originalOpenFollowUs = openFollowUsModal;
openFollowUsModal = function() {
  const pwaModal = document.getElementById('pwaInstallModal');
  if (pwaModal && pwaModal.style.display === 'flex') {
    return; // Don't open if PWA modal is showing
  }
  originalOpenFollowUs();
};